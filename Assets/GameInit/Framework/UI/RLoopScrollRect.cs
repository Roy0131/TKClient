...
{
    public abstract class RLoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler
                                          , IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public Action<int> mReturnAct { get; set; }
        public Func<int, bool, RectTransform> mCreateNewFunc { get; set; }

        //[HideInInspector]
        //public IRLoopProvider mLoopProvider;
        //protected List<UIBaseView> _lstViews;
        protected List<RectTransform> _lstTransform;
        ...
        protected RLoopScrollRect()
        {
            flexibleWidth = -1;
            //_lstViews = new List<UIBaseView>();
            _lstTransform = new List<RectTransform>();
        }
        ...
        {
            if (_lstTransform == null || _lstTransform.Count == 0)
                return;
            for (int i = _lstTransform.Count - 1; i >= 0; i--)
            {
                mReturnAct.Invoke(i);
                _lstTransform.Remove(_lstTransform[i]);
                //mLoopProvider.RetItemView(_lstViews[i]);
                //_lstViews.Remove(_lstViews[i]);
            }
        }

        public void RefillCells(int offset = 0)
        {
            if (!Application.isPlaying || mCreateNewFunc == null || mReturnAct == null)
                return;
        ...
        protected float NewItemAtStart()
        {
            ...
            {
                itemTypeStart--;
               // Debuger.Log("NewItemAtStart => contentConstraintCount:" + contentConstraintCount + ", itemTypeStart:" + itemTypeStart);
                RectTransform newItemView = InstantiateNextItem(itemTypeStart, true);
                newItemView.SetAsFirstSibling();
                _lstTransform.Insert(0, newItemView);
                size = Mathf.Max(GetSize(newItemView), size);
            }
        ...
        protected float DeleteItemAtStart()
        {
            ...
            if (((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && itemTypeEnd >= totalCount - 1)
                || content.childCount == 0 || _lstTransform.Count == 0)
                return 0;

            float size = 0;
            for (int i = 0; i < contentConstraintCount; i++)
            {
                //Debuger.LogWarning("DeleteItemAtStart => contentConstraintCount:" + contentConstraintCount + ", itemTypeStart:" + itemTypeStart);
                //UIBaseView oldItem = _lstViews[0];
                //size = Mathf.Max(GetSize(oldItem.mRectTransform), size);
                //mLoopProvider.RetItemView(oldItem);
                //_lstViews.Remove(oldItem);
                RectTransform oldItem = _lstTransform[0];
                size = Mathf.Max(GetSize(oldItem), size);
                mReturnAct.Invoke(0);
                _lstTransform.RemoveAt(0);
        ...
        protected float NewItemAtEnd()
        {
            ...
            {
                RectTransform newItemView = InstantiateNextItem(itemTypeEnd);//InstantiateNextItem(itemTypeEnd);
                size = Mathf.Max(GetSize(newItemView), size);
                _lstTransform.Add(newItemView);
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                //Debuger.LogWarning("contentConstraintCount:" + contentConstraintCount + ", i:" + i + ", _lstTransform.count:" + _lstTransform.Count);
                //UIBaseView oldItem = _lstViews[_lstViews.Count - 1];
                //size = Mathf.Max(GetSize(oldItem.mRectTransform), size);
                //mLoopProvider.RetItemView(oldItem);
                //_lstViews.Remove(oldItem);

                RectTransform oldItem = _lstTransform[_lstTransform.Count - 1];
                size = Mathf.Max(GetSize(oldItem), size);
                mReturnAct.Invoke(_lstTransform.Count - 1);
                _lstTransform.Remove(oldItem);

                itemTypeEnd--;
                if (itemTypeEnd % contentConstraintCount == 0 || content.childCount == 0 || _lstTransform.Count == 0)
                {
        ...
        private RectTransform InstantiateNextItem(int itemIdx, bool blHead = false)
        {
            RectTransform nextItem = mCreateNewFunc(itemIdx, blHead);//mLoopProvider.CreateNewItemView(itemIdx);
        ...
        void UpdateScrollbarLayout()
        {
            if (m_VSliderExpand)// && m_HorizontalScrollbar)
            {
                ...
                if (vScrollingNeeded)
                    m_HorizontalScrollbarRect.sizeDelta = new Vector2(-m_VSliderWidth, m_HorizontalScrollbarRect.sizeDelta.y);
                else
            ...
            if (m_HSliderExpand)// && m_VerticalScrollbar)
            {
                ...
                if (hScrollingNeeded)
                    m_VerticalScrollbarRect.sizeDelta = new Vector2(m_VerticalScrollbarRect.sizeDelta.x, -m_HSliderHeight);
                else
